...
public class DateUtils {
    ...
    public static LocalDateTime fromDate(Date date, ZoneId zoneId) {
        return LocalDateTime.ofInstant(date.toInstant(), zoneId == null ? ZoneId.systemDefault() : zoneId);
    }
    ...
    public static LocalDateTime fromLong(long timestamp, ZoneId zoneId) {
        return LocalDateTime.ofInstant(Instant.ofEpochMilli(timestamp),
            zoneId == null ? ZoneId.systemDefault() : zoneId);
    }

    public static LocalDateTime fromLong(long timestamp) {
        return fromLong(timestamp, ZoneId.systemDefault());
    }

    public static Date toDate(LocalDateTime time, ZoneId zone) {
        return Date.from(time.atZone(zone == null ? ZoneId.systemDefault() : zone).toInstant());
    }
    ...
    public static String now() {
        DateTimeFormatter sdf = getFormatter(DATE_FORMAT_NOW);
        return sdf.format(LocalDateTime.now());
    }

    /**
     * Returns the current date in a given format. DateUtils.now("dd MMMMM yyyy") DateUtils.now("yyyyMMdd")
    ...
    public static String now(String dateFormat) {
        DateTimeFormatter sdf = getFormatter(dateFormat);
        return sdf.format(LocalDateTime.now());
    }

    public static String format(Date time, String dateFormat) {
        DateTimeFormatter sdf = getFormatter(dateFormat);
        return sdf.format(fromDate(time));
    }

    public static String format(Long time, String dateFormat) {
        LocalDateTime l = fromLong(time);
        DateTimeFormatter sdf = getFormatter(dateFormat);
        return sdf.format(l);
    }

    public static String format(Date time) {
        return format(time, DATE_FORMAT_NOW);
    }
     ...
     *            milliseconds
     * @param isShowWeek
     *            是否采用周的形式显示 true 显示为3周前，false 则显示为时间格式mm-dd
     *
     * @return 如三周前，上午，昨天等
     */
