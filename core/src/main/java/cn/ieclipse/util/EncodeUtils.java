/*
 * Copyright (C) 2015-2016 ieclise.cn
 *
...
public final class EncodeUtils {
    ...
    public static String encodeUrl(String src, String charset) {
        ...
        try {
            return URLEncoder.encode(src, StringUtils.isEmpty(charset) ? UTF8 : charset);
    ...
    public static String decodeUrl(String encoded, String charset) {
        ...
        try {
            return URLDecoder.decode(encoded, StringUtils.isEmpty(charset) ? UTF8 : charset);
        } catch (UnsupportedEncodingException e) {
            return encoded;
        }
    }
     ...
     */
    public static String encodeRequestBody(Object input, String charset, boolean excludeNull) {
        if (input == null) {
            return null;
        ...
        for (int i = 0; i < ch.length; i++) {
            ...
            if (ch[i] == '\\' && j < ch.length && ch[j] == 'u') {
                ...
                if (j + 1 < ch.length && (i1 = getHexCharValue(ch[j + 1])) >= 0) {
                    code += i1 << 12;
                    int i2 = 0;
                    if (j + 2 < ch.length && (i2 = getHexCharValue(ch[j + 2])) >= 0) {
                        code += i2 << 8;
                        int i3 = 0;
                        if (j + 3 < ch.length && (i3 = getHexCharValue(ch[j + 3])) >= 0) {
                            code += i3 << 4;
                            int i4 = 0;
                            if (j + 4 < ch.length && (i4 = getHexCharValue(ch[j + 4])) >= 0) {
                                code += i4;
                                sb.append((char)code);
                                i += 5;
                            } else {
