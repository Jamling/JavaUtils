...
public class Patterns {
     ...
     *
     * @deprecated Due to the recent profileration of gTLDs, this API is expected to become out-of-date very quickly.
     *             Therefore it is now deprecated.
     */
    @Deprecated
    public static final String TOP_LEVEL_DOMAIN_STR = "((aero|arpa|asia|a[cdefgilmnoqrstuwxz])"
        + "|(biz|b[abdefghijmnorstvwyz])" + "|(cat|com|coop|c[acdfghiklmnoruvxyz])" + "|d[ejkmoz]" + "|(edu|e[cegrstu])"
        + "|f[ijkmor]" + "|(gov|g[abdefghilmnpqrstuwy])" + "|h[kmnrtu]" + "|(info|int|i[delmnoqrst])"
        + "|(jobs|j[emop])" + "|k[eghimnprwyz]" + "|l[abcikrstuvy]" + "|(mil|mobi|museum|m[acdeghklmnopqrstuvwxyz])"
        + "|(name|net|n[acefgilopruz])" + "|(org|om)" + "|(pro|p[aefghklmnrstwy])" + "|qa" + "|r[eosuw]"
    ...
    /**
     * Regular expression to match all IANA top-level domains for WEB_URL. List accurate as of 2011/07/18. List taken
     ...
     *
     * List accurate as of 2015/11/24. List taken from: http://data.iana.org/TLD/tlds-alpha-by-domain.txt This pattern
    ...
    static final String IANA_TOP_LEVEL_DOMAINS = "(?:"
        ...
        + "|express|e[cegrstu])" + "|(?:fage|fail|fairwinds|faith|family|fan|fans|farm|fashion|feedback|ferrero|film"
    ...
    private static final String PATH_AND_QUERY = "[/\\?](?:(?:[" + LABEL_CHAR + ";/\\?:@&=#~" // plus optional query
                                                                                              // params
        + "\\-\\.\\+!\\*'\\(\\),_\\$])|(?:%[a-fA-F0-9]{2}))*";
    ...
    public static final Pattern WEB_URL =
        Pattern.compile("(" + "(" + "(?:" + PROTOCOL + "(?:" + USER_INFO + ")?" + ")?" + "(?:" + DOMAIN_NAME_STR + ")"
    ...
    /**
     * Regular expression that matches domain names using either {@link #STRICT_HOST_NAME} or {@link #IP_ADDRESS}
     */
    ...
    private static final String WEB_URL_WITHOUT_PROTOCOL =
        "(" + WORD_BOUNDARY + "(?<!:\\/\\/)" + "(" + "(?:" + STRICT_DOMAIN_NAME + ")" + "(?:" + PORT_NUMBER + ")?" + ")"
    ...
    private static final String WEB_URL_WITH_PROTOCOL =
        "(" + WORD_BOUNDARY + "(?:" + "(?:" + PROTOCOL + "(?:" + USER_INFO + ")?" + ")" + "(?:" + RELAXED_DOMAIN_NAME
            + ")?" + "(?:" + PORT_NUMBER + ")?" + ")" + "(?:" + PATH_AND_QUERY + ")?" + WORD_BOUNDARY + ")";

    /**
     * Regular expression pattern to match IRIs. If a string starts with http(s):// the expression tries to match the
     * URL structure with a relaxed rule for TLDs. If the string does not start with http(s):// the TLDs are expected to
     * be one of the known TLDs.
     *
     * @hide
     */
    ...
    /**
     * Regular expression for local part of an email address. RFC5321 section 4.5.3.1.1 limits the local part to be at
     * most 64 octets.
     */
    ...
    /**
     * Regular expression for the domain part of an email address. RFC5321 section 4.5.3.1.2 limits the domain to be at
     * most 255 octets.
     */
    private static final String EMAIL_ADDRESS_DOMAIN = "(?=.{1,255}(?:\\s|$|^))" + HOST_NAME;

    /**
     * Regular expression pattern to match email addresses. It excludes double quoted local parts and the special
     * characters #&~!^`{}/=$*?| that are included in RFC5321.
     * 
     * @hide
     */
